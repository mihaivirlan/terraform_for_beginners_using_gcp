resource "google_cloud_run_service" "run-app-from-tf" {
  name = "run-app-from-tf"
  location = "asia-southeast1"

  template {
    spec {
      containers {
        #image = "gcr.io/google-samples/hello-app:1.0" //Update the container image to new below version (hello-app:2.0)
        image = "gcr.io/google-samples/hello-app:2.0"
      }
    }
  }

  // Follows I used "traffic" resource after updated the container image to new version (hello-app:2.0) to parted the containers traffic to 50% for each
  traffic {
    revision_name = "run-app-from-tf-n69l5"
    percent = 50
  }

  traffic {
    revision_name = "run-app-from-tf-dhbln"
    percent = 50
  }

}

resource "google_cloud_run_service_iam_policy" "pub_access" {
  service = google_cloud_run_service.run-app-from-tf.name
  location = google_cloud_run_service.run-app-from-tf.location
  policy_data = data.google_iam_policy.pub-1.policy_data
}

data "google_iam_policy" "pub-1" {
  binding {
    role = "roles/run.invoker"
    members = [ "allUsers" ]
  }
}
